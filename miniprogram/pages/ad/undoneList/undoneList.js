...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.getStorage({
      ...
      success: function (res) {
        ...
        console.log('super=>' + that.data._super)
        that.getData(res.data);
      },
      fail: function (res) { that.getData(that.data._super); },
    ...
    if (isadmin) {
      if (s) {
        wx.cloud.callFunction({
          ...
          // 成功回调
          complete: res => {
            console.log('undone order data is =》');
            console.log(res);
            wx.hideLoading();
            if (res.result != null) {
  ...
  sortByType: function (arrayList) {
    ...
    for (let i = 0; i < arrayList.length; i++) {
      switch (parseInt(arrayList[i].type)) {
        case 0:
  ...
  doSort: function () {
    let sortedList = this.sortByfloor(this.data.orderData)
    this.setData({ orderData : sortedList })
  },
  ...
  doSortByTime: function () {
    let arr = this.sortByTime(this.data.orderData)
    this.setData({ orderData: arr })
  },
  initSet: function () {
